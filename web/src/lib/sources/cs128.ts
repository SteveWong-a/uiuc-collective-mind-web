...
export function parseDueDate(text: any, now = new Date()) {
...
export function parseGradebook(html: string, { course, now = new Date() }: any) {
  ...
  for (const tr of doc.querySelectorAll("table tr")) {
    ...
    const url = href ? new URL(href, BASE).toString() : GRADEBOOK_URL;
